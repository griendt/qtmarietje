...
DEFAULT_HOST = 'noordslet.science.ru.nl'
DEFAULT_PORT = 1337
...
class RawMarietje:
	...
	def list_tracks(self):
		(id, timeStamp, length, time) = self.get_playing()
	...
	def upload_track(self, artist, title, user, size, f,update_signal=None):
		...
		while sent != size:
			...
			sent += toSent
			if update_signal:
                                update_signal.emit(float(sent)/float(size))
		l = s.recv(50)
		if l != 'UPLOAD::SUCCESS':
			raise MarietjeException, \
				"Unexpected reply: %s" % l
	...
	def run_fetch_songs(self):
		...
		try:
			...
			for (id, artist, title, flag) in self.raw.list_tracks():
				songs[id] = (artist, title,flag)
			sLoadTime = time.time() - starttime
			starttime = time.time()
			entries = list()
			for (id, artist, title, flag) in songs.iteritems():
